    ...
    if(self) {
        ...
        [[BeaconDefaults sharedDefaults].supportedProximityUUIDs enumerateObjectsUsingBlock:^(id uuidObj, NSUInteger uuidIdx, BOOL *uuidStop) {
            ...
            CLBeaconRegion *beacon = [[CLBeaconRegion alloc] initWithProximityUUID:uuid
                                                                       identifier:[NSString stringWithFormat:@"%@",uuid]];
            
            NSLog(@"Adding %@",[NSString stringWithFormat:@"%@",uuid]);
            [_rangedRegions addObject:beacon];
        }];
...
- (void)locationManager:(CLLocationManager *)manager didEnterRegion:(CLRegion *)region
{
    [self.delegates enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        [obj beaconTrackerEnteredRegion:region];
    }];
}
...
- (void)locationManager:(CLLocationManager *)manager didExitRegion:(CLRegion *)region {
    ...
    [self.delegates enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        [obj beaconTrackerLeftRegion:region];
    }];
}
